...
from schemas import CarInput, CarOutput, CarInput, Trip, TripInput, User
...
def get_cars(size: str | None = None, doors: int | None = None,
    ...
    query = select(CarInput)
    if size:
        query = query.where(CarInput.size == size)
    if doors:
        query = query.where(CarInput.doors >= doors)
    return session.exec(query).all()


@router.get("/{id}", response_model=CarOutput)
def car_by_id(id: int, session: Session = Depends(get_session)) -> CarInput:
    car = session.get(CarInput, id)
    if car:
        return car
    else:
...
@router.post("/", response_model=CarInput)
def add_car(car_input: CarInput,
            session: Session = Depends(get_session),
            user: User = Depends(get_current_user)) -> CarInput:
    new_car = CarInput.from_orm(car_input)
    session.add(new_car)
    session.commit()
...
@router.put("/{id}", response_model=CarInput)
def change_car(id: int, new_data: CarInput,
               session: Session = Depends(get_session)) -> CarInput:
    car = session.get(CarInput, id)
    if car:
...
def add_trip(car_id: int, trip_input: TripInput,
    ...
    car = session.get(CarInput, car_id)
    if car:
